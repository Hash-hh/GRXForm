...
# CHANGED: Function now accepts 'dataloader' instead of 'dataset'
def train_for_one_epoch(epoch: int, config: MoleculeConfig, network: MoleculeTransformer,
                        optimizer: torch.optim.Optimizer, dataloader: DataLoader, is_validation=False):
    # REMOVED: DataLoader creation is now done in main() to allow persistent_workers=True
...
if __name__ == '__main__':
    pretrain_train_dataset = "./data/zinc/pretrain_sequences/zinc_train.pickle"
    pretrain_val_dataset = "./data/zinc/pretrain_sequences/zinc_valid.pickle"
    ...
    num_dataloader_workers = 15  # Number of dataloader workers for creating batches for training
    load_checkpoint_from_path = './results/pretrain/last_model.pt'
    ...
    if pretrain_num_epochs > 0:
        ...
        if checkpoint["optimizer_state"] is not None and load_optimizer_state:
        ...
        # CHANGED: Created DataLoaders OUTSIDE the loop with persistent_workers=True
        # This fixes the crash and improves speed.
        ...
        for epoch in range(pretrain_num_epochs):
            ...
            generated_loggable_dict = train_for_one_epoch(
                epoch, config, network, optimizer, train_dataloader  # CHANGED: Passed dataloader
            )
            checkpoint["pretrain_epochs_trained"] += 1
            scheduler.step()
            print(
                f">> Epoch {checkpoint['pretrain_epochs_trained']}. Avg loss level 0: {generated_loggable_dict['loss_level_zero']},"
            ...
            if config.use_wandb:
                wandb.log(generated_loggable_dict, step=epoch)

            print("Validating...")
            torch.cuda.empty_cache()
            with torch.no_grad():
                validation_metrics = train_for_one_epoch(
                    None, config, network, None, val_dataloader, is_validation=True  # CHANGED: Passed dataloader
                )
